...
function extractBrandMentions(text, userBrand) {
    ...
    .forEach((line) => {
      ...
      if (m) {
        ...
        if (wm) {
          ...
          if (!brandMap[brand])
            brandMap[brand] = { brand, position: rank, mentions: 1 };
  ...
  if (pos !== -1 && !brandMap[userBrand])
    brandMap[userBrand] = {
      brand: userBrand,
      position: Math.ceil((pos / text.length) * 10) + 1,
      mentions: 1,
    };

  Object.keys(brandMap).forEach((b) => {
    const hits = text.match(
      new RegExp(b.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "gi"),
    );
...
function calcScore(mentions, userBrand) {
  const found = mentions.find((m) => isMatch(m.brand, userBrand));
  ...
  return Math.min(
    100,
    Math.round(
      Math.max(0, 100 - (found.position - 1) * 15) +
        Math.min(found.mentions * 5, 20),
    ),
  );
}
...
function isMatch(mBrand, userBrand) {
  const a = normalize(extractMainBrand(mBrand));
  const b = normalize(userBrand);

  return a.includes(b) || b.includes(a);
}
function extractMainBrand(str) {
  return str.split("-")[0]; // "Doctor's Best"
}
    ...
    return [
      {
        ...
        description:
          "Use keyword-rich titles matching how AI describes top products.",
        impact: "high",
        effort: "medium",
      },
      {
        title: "Build review volume",
        description: "AI models favor 500+ reviews with 4.5+ rating.",
        impact: "high",
        effort: "high",
      },
      {
        ...
        description:
          "Get listed on Wirecutter, BestReviews — AI pulls from these.",
        impact: "high",
        effort: "medium",
      },
      {
        title: "Answer Amazon Q&A",
        description: "Thorough Q&A answers get pulled directly by AI models.",
        impact: "medium",
        effort: "low",
      },
    ];
...
app.post("/api/analyze", async (req, res) => {
  ...
  const modelResults = rawResults.map((r) => {
    ...
    return {
      ...
      topBrands,
      userScore,
      userRank: idx === -1 ? null : idx + 1,
      userFound: idx !== -1,
    };
  });

  const valid = modelResults
    .filter((r) => r.userScore !== null)
    .map((r) => r.userScore);
  const overallScore = valid.length
  ...
  modelResults.forEach((r) => {
    r.topBrands.forEach((b) => {
      const isUser =
        ...
        userBrand.toLowerCase().includes(b.brand.toLowerCase());
      if (!isUser) {
        if (!competitorMap[b.brand])
          competitorMap[b.brand] = {
            brand: b.brand,
            modelCount: 0,
            positions: [],
          };
  ...
  const topCompetitors = Object.values(competitorMap)
    ...
    .sort(
      (a, b) => b.modelCount - a.modelCount || a.avgPosition - b.avgPosition,
    )
    .slice(0, 5);
